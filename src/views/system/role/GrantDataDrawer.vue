...
<script lang="ts">
  ...
  import { ownDeptList, grantRoleData } from '/@/api/system/role';
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      async function InitOwnOrgData(dataScope) {
        if (dataScope === 5) {
          owndeptData.value = await ownDeptList(rowId);
          unref(treeAction)?.filterByLevel(1);
        } else {
          owndeptData.value = [];
        }
      ...
      async function handleSubmit() {
        try {
          ...
          values.deptIdList = unref(treeAction)?.getCheckedKeys();
          values.id = rowId;
